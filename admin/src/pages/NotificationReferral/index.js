...
function NotificationReferralPage() {
  ...
  const getPagingLogs = () => {
    getNotificationReferral().then((res) => {
  ...
  const handlePostHome = async (values) => {
    ...
    const rs = await updateNotificationReferral(values);
    if (rs?.status === 1) {
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            ...
            pageTitle="Quản lý Điều Kiện Rút Tiền"
          />
          <Row className="mb-3">
...
export default NotificationReferralPage;
