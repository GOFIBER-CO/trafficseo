...
const ModalViewHistoryAllTime = ({ id }) => {
  ...
  const exportExcel = async () => {
    ...
    const result = await getPointLogs(dataReq);
  ...
  const getLogs = async (pageSize, pageIndex) => {
    ...
    const res = await getPointLogs(dataReq);
    setLoading(false);
    setLogs(res.data);
          ...
          <div style={{ display: "flex", width: "100%", flexWrap: "wrap" }}>
            {Object.keys(moneyByBrand)?.map((item) => (
              <div
                ...
                style={{ width: "230px" }}
              >
                <span style={{ fontWeight: 500 }}>Người dùng {item}</span>
                <span>
...
export default ModalViewHistoryAllTime;
