...
import "../../App.css";
import { getNotification, updateNotification } from "../../helpers/helper";
...
function NotificationHome() {
  ...
  const handlePostHome = async (values) => {
    ...
    try {
      const rs = await updateNotification(values);
      if (rs?.status === 1) {
        return toast.success("Lưu thành công");
      } else {
        return toast.error("Lưu thất bại");
      }
    } catch (error) {
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            ...
            pageTitle="Quản lý Thông Báo Trang Chủ"
          />
          <Row className="mb-3">
