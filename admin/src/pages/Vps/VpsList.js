...
function VpsList() {
  ...
  const getVps = () => {
    getPagingVps(pageSize, pageIndex, filter).then((res) => {
      setVps(res.data);
  ...
  const getVpsTabs = async () => {
    let domains = await getPagingVpsTabs(99999, 1, "");
    setTabList(domains.data);
  };
  ...
  const onFinish = async (data) => {
    const dataReq = {
      ...
      advance: advance?.map((item) => item.text) || [],
      isBest,
      tab,
    };
    ...
    if (!data.id) {
      ...
      const dataRes = await addVps(dataReq);
      ...
      //Update
      const dataRes = await updateVps(data.id, dataReq);
      dataRes.success === true
  ...
  const onEdit = (key) => {
    const dataEdit = vps.filter((item) => item._id === key);
    ...
    setAdvance(
      (dataEdit[0]?.advance || []).map((item, index) => {
        return {
          id: index,
          text: item,
        };
  ...
  const onRemoveAdvance = (id) => {
    const newAdvance = advance.filter((item) => item.id !== id);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Vps" pageTitle="Quản lý Service" />
          <Row className="mb-3">
            <Drawer
              title={drawerTitle}
              placement={"right"}
                ...
                <Row>
                  ...
                  <Col>
                    ...
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: true,
                          message: "Vui lòng nhập vps slug!",
                        },
                        {
                          type: "slug",
                        },
                  ...
                  <Col xs={12} className="mb-2">
                    ...
                    <Input
                      ...
                      placeholder="Nhập xong nhấp enter"
                      type="text"
                      onKeyPress={onPressAdvance}
                    />
                    {advance.map((item, i) => (
                      <TagComp
                        key={item.id}
                        content={item}
                        onRemove={onRemoveAdvance}
                        id={item.id}
                      />
                    ))}
                  </Col>
                  ...
                  <Col>
                    ...
                    <Form.Item
                      name="tab"
                      rules={[
                        { required: true, message: "Vui lòng chọn Vps tab" },
                      ]}
                    >
                      <Select
                        size="large"
                ...
                <Column
                  ...
                  render={(val, record) => (
                    <Space size="middle">
                      ...
                      <Popconfirm
                        title="Are you sure to delete this vps?"
                        onConfirm={() => {
                          deleteVps(val._id).then(() => {
                            getVps(pageSize, pageIndex);
                          });
                        }}
                        okText="Yes"
