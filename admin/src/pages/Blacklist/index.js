...
function Blacklist() {
  ...
  const onEdit = (key) => {
    const dataEdit = blacklist.filter((item) => item._id === key);
    form.setFieldsValue({
      ip: dataEdit[0].ip,
    ...
    showDrawer();
    setDrawerTitle("Chỉnh Sửa Blacklist");
  };
  const onDelete = async (key) => {
    const result = await removeBlacklist(key);
  ...
  const handleImportFile = async (data) => {
    ...
    const oldData = await getBlacklist(dataReq);
    data &&
      data.map((item) => {
        ...
        if (check.length === 0) {
          ...
          if (check[0]?.note !== item.note) {
            ...
            editBlacklist(id[0]._id, updatedIp);
          }
        }
      });
  };
  ...
  const exportExcel = async () => {
    ...
    const listBlacklist = await getBlacklist(dataReq);
    const fileType =
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8";
    const fileExtension = ".xlsx";
    const blacklistExcel = listBlacklist?.data?.map((ip, index) => {
      return {
        STT: index + 1,
        "Địa chỉ IP": ip.ip,
        "Ghi chú": ip.note,
    ...
    const ws = XLSX.utils.json_to_sheet(blacklistExcel);
    const wb = { Sheets: { data: ws }, SheetNames: ["data"] };
    const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    const data = new Blob([excelBuffer], { type: fileType });
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Blacklist" pageTitle="Quản lý Blacklist" />
          <Row className="mb-3">
            <Drawer
              title={drawerTitle}
              placement={"right"}
