...
export default function TeamManagement() {
  ...
  const getData = async () => {
    const brands = await getPagingTeam(searchInput, pageSize, pageIndex, sort);
  ...
  const deleteBrandById = async (val) => {
    try {
      await deleteTeam(val?._id);
      getData();
      return toast.success("Xóa team thành công!");
    } catch (error) {
      return toast.error("Xóa team lỗi!");
    }
  };
  useEffect(() => {
    getData();
    ...
    <React.Fragment>
      <Spin spinning={false}>
        <div className="page-content">
          <Container fluid>
            <BreadCrumb title="Danh sách team" pageTitle="Quản lý team" />
            <Row className="mb-3">
            ...
            <Row>
              <Col lg={12}>
                <Table dataSource={data} pagination={false}>
                  <Column title="Tên team" dataIndex="name" key="name" />
