...
export default function Login() {
  ...
  useEffect(() => {
    document.addEventListener("connected_extension", (e) => {});
    const connect_extension = new CustomEvent("connect_extension", {
      data: "adawd",
    });

    const test1 = new CustomEvent("test1", { data: "adaw222d" });
    document.dispatchEvent(test1);
    document.dispatchEvent(connect_extension);
  }, []);
  useEffect(() => {
    if (referralCode) {
      const getCode = localStorage.getItem("code");
      if (!getCode) localStorage.setItem("code", referralCode);
    }
  ...
  const loginGoogle = useGoogleLogin({
    ...
    onSuccess: (token) => loginWithGoogle(token.access_token, referralCode),
    overrideScope: true,
  });
    ...
    <div className="container d-flex align-items-center w-100 min-vh-100 justify-content-center">
      {loading || (!loading && user) ? (
        ...
        <form className={styles["form_container"]}>
          ...
          {code && (
            <div className={styles["input_container"]}>
              <label className={styles["input_label"]} htmlFor="code_referral">
                Mã giới thiệu
              </label>
              <AiOutlineQrcode className={styles["icon"]} size={24} />
              <input
          ...
          <a
            ...
            href="https://t.me/sofm1991"
          >
            <img src="/help.png" width={22} height={22} />
          ...
          {/* <button
            ...
            onClick={(events) => {
              events.preventDefault();
              toast.warning("Chức năng này đang bảo trì!");
            }}
          >
            <ImFacebook2 />
          ...
          <Link href={"/register"} className="w-100">
            ...
            >
                <span>Đăng ký BTV (SEO/CONTENT OKVIP)</span>
            </button>
          </Link>
